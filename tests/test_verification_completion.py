...
class FastVerificationTests(unittest.TestCase):
    ...
    def test_cli_spawn_resume_and_corruption_rejection(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            verified = command('validate_r2_budget_grid.py', '--output', output, '--workers', 2,
            ...
            for script, args in (
                ...
                ('validate_r2_budget_grid.py', ['--summaries-only', '--verification-backend', 'auto']),
            ):
            ...
            rejected = command('validate_r2_budget_grid.py', '--output', output, '--workers', 1,
                               '--verification-backend', 'numba')
