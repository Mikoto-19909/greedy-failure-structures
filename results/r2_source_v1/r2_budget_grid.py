...
def _analyze(output, budget):
    output = Path(output)
    design = validate_design(read_json(output / "config.json"))
    records = load_records(output, design)
    verified = read_json(output / "verification.json")
    if verified.get("status") != "passed" or set(verified["graphs"]) != {r["task"]["base_graph_id"] for r in records}:
        raise ValueError("analysis requires complete independent graph verification")
