...
实际起点为生成器 [PR #40](https://github.com/Mikoto-19909/greedy-failure-structures/pull/40)
合并提交 `88962896de53d4f0fa9ad8b931747dd8158a0613`。从该 Git 树独立导出旧源码，
用 Python 3.12.3 冻结九类的 40 条合法记录、九份真实 CSV、protocol 4/5 旧 pickle，
以及字段、默认值、公开身份、不可变性、消费者和 124 条实际拒绝记录。
旧文件与源码哈希保存在本地 `results/_contracts_split/frozen-records/manifest.json`；
该基线先于任何记录迁移生成，后续不能通过重生成旧数据接受差异。

按用户后续授权，三组分别机械提交，全部完成后集中进行一次兼容、真实 spawn、
类型检查和独立审查。复用未变化的验证结果，重复本地全量门禁由当前 PR 的必需
检查覆盖。本节的实施安排替代原逐组重复验收步骤。

三组分别由 `b53a74c`、`3fc3c44`、`aa6683b` 迁移到
`_performance_association_contracts.py`、`_performance_contracts.py` 和
`_quality_contracts.py`。`contracts.py` 与 `_benchmark_result.py` 的实际导入已同步；
没有保留无人使用的旧私有导出。类原文、schema 常量、装饰器和公开 `__module__`
赋值保持原样，包根与公开聚合入口的名称、顺序和对象身份不变。

最终一次新解释器比较通过：全部字段、默认值、空值、错误、不可变性和消费者
...
- 支持的公开名称、包根导出顺序、公开入口的类对象身份关系；区分有意公开的名称与偶然泄漏的实现名称。
...
只绑定旧私有路径或偶然实现名称的断言可以移除或改为行为检查；
包根公开导出顺序及其现有兼容测试保留。
有意公开的接口不能借此删除，真正需要收缩公开 API 时应另行说明兼容影响。
历史迁移清单、配置和冻结研究产物保持原样。
