...
记录日期：2026-09-05。状态：四组源码已拆分，进入最终集中验收。
...
共享工具已在独立提交 `78892b7` 原样迁移。固定提交导出的候选树与旧矩阵在所有
调用、API、错误和 pickle 行为上相同；候选同时读取旧 pickle 并写入两种协议的新
pickle。外置核验器另行检查旧 pickle 清单哈希、实际 blob 与候选 Git 树，避免仅凭
比较结果中的自报信息认定来源一致。

下一提交原样迁移对抗组五个函数，入口保留两个公开函数的显式重新导出。
唯一已发现的私有测试导入 `_potential_distractor_rankings` 随实现转到新模块。
注册表、其他构造和 `from_spec()` 保持原文；每组的源码、作用域、导入身份与类型
检查先通过，再对固定候选版本重放同一旧矩阵。

对抗组 `e5a37de` 已通过完整旧矩阵、旧/新 pickle 与来源核验，相关对抗测试通过。
受控组随后原样迁移四个公开构造和 `_controlled_element_order`，各自仍使用原命名
空间及固定抽样池大小。公开入口直接重导出，其他构造和注册表继续保持原文。

最后迁移普通随机组的公开构造与专用工具，入口保留原注册表、`from_spec()`、参数
常量及公开函数重导出。原临时共享工具导入和无用标准库导入已移除，实际私有调用方
已随所属实现调整。所有原函数、嵌套作用域及实际全局绑定保持一致。

按用户后续授权，已通过的源码、类型和分组结果直接复用，最终版本集中进行一次
剩余兼容验收；不再逐阶段重复全矩阵、整套测试或独立审查。GitHub 必需检查仍执行，
许可证清单按最终暂存内容更新。该调整减少重复执行，不缩小最终兼容要求。
...
PR #21 涉及有效 coupling、manifest 文件绑定和 schema 校验。
若已合并，运行该版本的配对分析测试；若仍未合并，明确记录分支范围，
需要检查交互时在临时工作树合并目标代码，不把另一个 PR 的提交夹带进本 PR。
