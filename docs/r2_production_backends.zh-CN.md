...
首版实现、真实 GPU 检查及全量性能结果见[加速成果整理](../analysis/acceleration_delivery.zh-CN.md)。
...
及枚举时间，完成事件只在图保存后写入。
处理原始失败时，若失败事件也因 I/O 错误无法写入，保留原异常，并在异常附注中显示
日志错误。正常完成事件写入失败仍中止运行；恢复根据已保存原图判断哪些任务可以复用。
...
可选跳过，不能据此声称 GPU 已验证。跨图批处理和自动 GPU 调度不属于首版。
