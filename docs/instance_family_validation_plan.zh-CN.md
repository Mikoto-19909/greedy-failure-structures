...
上述为 2026-09-11 的实现与验收记录。本次将候选分支同步到已交付主线后提交 PR 供审查；是否合并仍待审查决定，不据此启动后续跨文件拆分。

## PR 主线整合验证（2026-09-13）

与主线 `dcfd2221` 整合后，默认检查运行 518 项：513 项通过、5 项按既有条件跳过，mypy 检查 42 个源码文件通过。跳过项为可选 Matplotlib、OR-Tools、两项显式 CUDA 检查与 POSIX 专用虚拟环境检查，必需研究验证均执行。

独立复核将家族函数展开回调用位置并还原参数名，整个模块 AST 与主线基线一致；另用 12 个代表记录、23 组变异与 4 个入口或状态观测完成 1,104 组旧新版比较：164 接受、940 拒绝、差异为 0。公开身份、38 个字段、规范化、CSV、异常类型/信息及输入不变性均在该有限样本中一致；6 项聚焦回归也通过。没有发现阻塞问题，未重跑研究实验。
