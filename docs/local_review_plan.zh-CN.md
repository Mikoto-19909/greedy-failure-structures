...
本轮授权范围为定位 HTTP 错误、完成 full 验收、建立 Draft PR 并核对当前提交的
CI/Linux 回归与审阅反馈。三次真实 PR 效果试用仍另行进行。

对修复前 `8f1ac54` 与修复后 `3ca923f`，分别固定运行问题用例 20 次及整个 HTTP
测试类 2 次，均通过；两提交的 Dashboard 实现和 HTTP 测试相同。
独立分包实验稳定重现原因：服务端按请求头提前拒绝并关闭，客户端随后发送 body，
5/5 次读取响应出现同一 WinError 10053；合法分包请求 5/5 返回 200，仅发送无效头部
5/5 返回 415。另以非零 Content-Length 且不发 body 验证 403/415，确认 body 解析与任务派发均未调用。
这是测试客户端与提前拒绝的连接关闭时序冲突，不是引用传递回归。

四类拒绝头部策略的测试改为只发 headers，并声明非零 Content-Length，明确验证拒绝
