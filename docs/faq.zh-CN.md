...
和报告逻辑，而完整 benchmark 产物会在本地写入 `results/`。公开结论所需的最小
冻结证据位于 `experiments/core_rq/`，其权威映射记录在
[`CLAIMS.md` 台账](../experiments/core_rq/CLAIMS.md)。面向外部的研究叙述位于
[`analysis/`](../analysis/README.md)。当前 claim ledger 尚未包含定量研究结论。

<!-- faq:id=content-boundary -->

## 为什么要强制执行内容边界？

本仓库允许发布少量经过验证的核心研究结论。内容边界要求完整探索输出留在本地、
最小冻结证据集中进入一个公开目录，并让每个定量研究结论都能回到 claim ledger
中的对应条目。CI 允许定量文字通过，但不会验证人工维护的证据映射。
