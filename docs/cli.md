...
The service home `/` offers two study entries: `/maximum-coverage` for the existing
Maximum Coverage tools and `/online-matching` for the imported independent case.
`/index.html` remains a Maximum Coverage alias. Opening either page starts no computation.
Online matching runs fixed inputs through its own runner; it does not use the
Maximum Coverage queue described below.
...
On the Maximum Coverage page, choose **Use introductory example**, check the planned size,
...
comparison and metric explanations before charts and a readable full report.
The shared result picker precedes the spectrum and comparison, with direct navigation
to the comparison, charts, and replay. The last successfully loaded choice survives
reloads. Without a saved choice, the most recently updated readable CSV result is
selected. Missing or unreadable saved choices produce a notice and fall back to other
results. The timestamp describes CSV file updates, not experiment completion.
Failed switches clear the previous display instead of leaving unrelated data visible.
...
Result names use 1–81 characters, beginning with an ASCII letter or digit, followed
by letters, digits, dots, underscores, or hyphens. Inline validation prevents invalid
names from starting a run. The latest job status is separate from submission errors.

Saved execution-issue files come from timeouts or execution errors, not every greedy
optimality gap. Replay defaults to the current result; **Other saved cases** includes
folders without CSV reports. Empty scopes explain why replay controls are disabled.
Changing the case or result clears the previous replay output.

Reports open inside the application with headings, paragraphs, lists, and tables.
The original language and limitations are preserved, with a return button and an
original Markdown download. The viewer supports the constructs generated by the
project; other syntax remains text and report HTML is not executed. When no headline
section exists, the viewer offers the full report without inventing a summary.

#### Adding a research entry

Home cards and study navigation share [topics.json](../src/maxcover/dashboard_ui/topics.json).
Array order controls display order. Copy an entry and set a unique `id`, a local
page `href`, a short `mark`, and bilingual `title`, `role`, `summary`, and `features`.
The first three text fields have `zh`/`en` strings; `features` has `zh`/`en` string arrays.
This list only describes navigation. Each study supplies its own data and execution logic.

An existing page needs only a new list entry. A new page also needs its page/assets
added to `_STATIC_FILES` in `dashboard.py`. Include `/topics.css` and `/topics.js`,
then add a navigation container whose current ID matches the list entry:

```html
<nav data-topic-nav data-topic-current="your-study"><a href="/">Research home</a></nav>
```

Cards adapt to the available width; a dropdown keeps navigation compact as entries grow.
No card or switcher renderer changes are needed. Check entry navigation, the selected
study, return to home, and narrow screens. Browsing must not start an experiment.
...
selected-set coverage, and optimum/gap consistency before computing differences:
...
References come from optimal exact-run records or certificates regenerated from
the configured instances; unavailable references remain unknown. These checks
do not rerun exact solvers.
