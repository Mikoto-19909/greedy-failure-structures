...
PR 审查后另补单故障回归：在 schema 有效时分别检查 outputs 形状和缺少必需声明，
并在标题有效时单独检查刷新摘要后的运行时间图表。多故障用例继续保留。
独立复核确认移除任一对应检查都会使新增测试失败；测试还区分受控验证失败与
同样退出 1 的未处理异常，只约束必要类别和文件定位，不固定整句诊断措辞。
