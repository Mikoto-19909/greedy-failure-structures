...
服务首页 `/` 提供最大覆盖研究与有限改派在线匹配两个入口。
最大覆盖页面位于 `/maximum-coverage`，原 `/index.html` 地址继续可用。
在线匹配位于 `/online-matching`，支持报告阅读、分配回放和固定输入复现。
在线匹配使用自己的运行入口，目前不进入下述最大覆盖任务队列。
...
同一服务的 `/research` 提供 R2–R4 保存产物查看和持久化本地任务中心。
Benchmark、反例挖掘和猜想检验进入同一串行队列；原页面的“运行实验”也使用此队列。
Benchmark 可查看已保存进度、请求暂停/取消，并从同目录冻结配置续跑，已有 run_id 不重算。
暂停/取消会等待当前计算与工作进程收尾；运行中结果文件暂不开放读取，以保护检查点原子写入。
算法错误、超时、计算完成和独立验证分别说明，Benchmark 完成不自动表示科研验证通过。比较页可保存
当时的完整筛选数据，并下载 CSV、Markdown、SVG 和 JSON。数据与日志都保留在本机，
当前不考虑上云；路径、输入预算和复验方式见[模块拓展计划与使用记录](dashboard_expansion_plan.zh-CN.md)。

本地读取索引按需建立，源文件变化后更新；工作台提供状态与重建按钮。已结束任务和
保存分析可用“归档／还原”调整列表，“包含已归档”查看历史；这些操作不删除或移动
原始结果。索引首次建立会增加耗时与磁盘占用，具体测量和恢复边界见同一模块记录。

`/experiments` 可复制模板为本地配置、编辑基本参数/完整 JSON、校验规模并查看参数差异。
从“单组入门模板”可直接开始：版本 3、单组均匀随机、无扫描且仅含贪心、惰性贪心或穷举的配置，
支持表单编辑元素数、集合数、选择名额、密度和算法启用状态；其他配置说明原因并保留文本编辑。
算法停用保留原条目与选项。表单修改后点击“应用表单并检查”，通过后才能保存；修改完整 JSON 后先检查再保存。
未保存修改不能交接到运行中心。交接携带保存时的配置身份，语义已变时停止提交并要求重新核对。
仅修改排版不改变配置身份；原保存冲突检查仍会识别文件字节变化。
实验库支持主题、标签和备注。模板保持只读，副本在 `configs/local/`，旧版本编辑返回冲突。
从专题详情进入 `/research-analysis?source=...` 可看预算曲线、分布、合法同图配对，并按已登记
的本地目录或固定 Git 证据打开原图/端点、查看 Greedy 步骤和导出回放。详见
[事项 1、5、2、4 的使用与验收](dashboard_selected_scope_20260921.zh-CN.md)。备份与进一步规模优化仍暂缓。

进入最大覆盖页面后，可点击“选用入门示例”，确认运行规模后点击“运行实验”；选择示例本身不会启动计算。
...
首页卡片与页面内专题切换共用 [topics.json](../src/maxcover/dashboard_ui/topics.json)。
数组顺序就是显示顺序。新增专题时，复制一条记录，填写唯一的 `id`（专题标识）、
`href`（站内页面地址）、`mark`（短标记），以及中英文的标题、用途、说明和功能列表。
`title`、`role`、`summary` 的 `zh`/`en` 值是文本；`features` 的对应值是文本数组。
此配置只描述入口，各专题自行提供实验和数据处理功能。

已有页面只需新增配置记录。新页面还需在 `dashboard.py` 的 `_STATIC_FILES` 中加入实际页面和资源路由。
页面引入 `/topics.css`、`/topics.js`，并放置以下导航容器；专题标识要与配置对应：

```html
<nav data-topic-nav data-topic-current="your-study"><a href="/">研究首页</a></nav>
```

卡片按可用宽度自动排列，专题切换使用下拉框。新增记录不需要修改这两处的渲染代码。
添加后核对首页进入、当前专题选择、返回首页和窄屏显示；浏览入口不得自动执行实验。
