(() => {
  ...
  const I18N = {
    zh: {
      ...
      "gap.title": "贪心在哪里失败",
      "gap.lede":
        "每条光带对应一个案例族，长度是该族上贪心算法与最优解的平均差距。光带越长，结构越会诱导贪心犯错。",
      "gap.noSource": "尚未选择结果集",
      "gap.empty": "运行一个 benchmark 之后，光谱会从这里长出来。",
      "gap.note":
        "条带长度 = 贪心类算法与最优解的平均差距（mean_optimality_gap），按差距从大到小排列。",
      "gallery.ariaLabel": "成果画廊",
      "gallery.kicker": "成果 · 结果画廊",
      "gallery.title": "结果画廊",
      "gallery.lede": "每张图是当前结果集的一份报告，装裱成作品挂在墙上。",
      "gallery.empty":
        "运行完成后，报告图表会挂上这面墙。先在上方启动一个实验。",
      "gallery.summaryKicker": "对照 · 摘要",
      "gallery.summaryTitle": "算法对照",
      "gallery.loadingSummary": "正在读取报告摘要…",
      "gallery.openReport": "打开完整报告 ↗",
      "gallery.zoomHint": "点击放大",
      "lightbox.close": "关闭",
      "lightbox.escHint": "Esc 关闭",
      "results.updated": "更新时间 {time}",
      "results.failureCount": "失败案例 {count} 个",
      ...
      "replay.outputEmpty": "运行后显示结果和匹配状态。",
    ...
    en: {
      ...
      "entry.title": "Start a run",
      "entry.lede":
        "Pick a configuration and run it locally. When it finishes, the results hang on this wall.",
      "fields.configFile": "Configuration",
      "fields.outputName": "Result folder name",
      "fields.outputPlaceholder": "e.g. quick-dashboard",
      "fields.forceRun": "Clear and rerun",
      "actions.runBenchmark": "Start benchmark",
      "actions.refreshResults": "Refresh",
      "actions.runReplay": "Run replay",
      "plan.instances": "Total instances",
      "plan.repetitions": "Repetitions per instance",
      "plan.runs": "Planned algorithm runs",
      "run.idle": "Idle · job status appears here.",
      ...
      "gap.lede":
        "Each band is a case family; its length is the average gap between greedy and optimal. Longer bands mark structures that lead greedy astray.",
      "gap.noSource": "No result set selected",
      "gap.empty": "Run a benchmark and the spectrum grows here.",
      "gap.note":
      ...
      "gallery.title": "Result gallery",
      "gallery.lede":
        "Each image is one report from the selected result set, framed and hung on the wall.",
      "gallery.empty":
        "After a run completes, report charts hang on this wall. Start a run above first.",
      "gallery.summaryKicker": "TABLE · COMPARISON",
      "gallery.summaryTitle": "Algorithm comparison",
      "gallery.loadingSummary": "Loading report summary…",
      "gallery.openReport": "Open full report ↗",
      "gallery.zoomHint": "Click to enlarge",
      "lightbox.close": "Close",
      "lightbox.escHint": "Esc to close",
      "results.updated": "Updated {time}",
      "results.failureCount": "{count} failure cases",
      ...
      "replay.outputEmpty": "The result and match status appear here.",
  ...
  function summaryFrame(artifact, hero) {
    ...
    pre.textContent = t("gallery.loadingSummary");
    ...
    fetch(artifact.url)
      ...
      .then((text) => {
        pre.textContent = text.slice(0, 800);
      })
      .catch(() => {
        pre.textContent = "";
      });
  ...
  async function refreshAll() {
    try {
      ...
      setSelect(
        "#config-select",
        state.configs.map((item) => ({ label: item.name, value: item.path })),
        t("config.noConfigs"),
      );
      setSelect(
        "#replay-algorithm",
