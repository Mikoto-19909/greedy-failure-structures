...
阅读结论从[最终报告](r4_final_research_report.zh-CN.md)开始；恢复已发布输入、证书及
执行源码，按文末固定快照中的 `RESTORE.md` 操作。已有完整比较无需再运行预检。
下节保留原预检与比较命令；完整重算使用新输出目录，续跑使用原目录和 `--resume`，
均须遵守固定配置及累计预算。仅恢复配置不代表当前源码可以执行冻结比较：
入口还检查 `code_revision` 对应的计算依赖，包括整个 `src/maxcover`。
需要历史复现时使用档案指定源码及环境，不修改冻结配置绕过该检查。
[稳定化阶段](../docs/stabilization_plan.zh-CN.md)不自动启动这些实验。
