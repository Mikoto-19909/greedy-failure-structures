...
完整原图、链、交换记录、端点和见证见 [graphs](../results/r3_confirmation_v1/graphs/)。
[端点表](../results/r3_confirmation_v1/endpoint_results.csv)保留全部 12,000 个端点，
[原图级表](../results/r3_confirmation_v1/base_graph_summary.csv)有 3,000 行，
[主汇总](../results/r3_confirmation_v1/primary_summary.json)保存未舍入估计及区间。
15,000 个原图/链种子互不重复，且与 R2、R3 探测和历史输入分离，见[执行前核对](../results/r3_confirmation_readiness.json)。
...
环境为 Python 3.12.14、NumPy 2.3.5、SciPy 1.18.1，详见[运行环境](../results/r3_confirmation_v1/run_context.json)。
正式生产约 33.91 秒，原图/交换/最优参考独立验证约 33.38 秒，汇总前当前数据复核约 32.33 秒，
派生表独立核对约 0.17 秒，累计约 **99.79 秒**，在 F3 的 1 小时计算预算内。
这是当前机器的执行记录，不是通用性能保证；实际记录见[执行日志](../results/r3_confirmation_v1/execution.jsonl)。

[原图验证](../results/r3_confirmation_v1/verification.json)重放所有提议、接受决定、度序列，
并独立枚举每个原图/端点的 O/O1 和规范见证；[派生验证](../results/r3_confirmation_v1/summary_verification.json)
...
见[检查日志](../results/r3_confirmation_full_check.log)。独立复核者为代理，实际核对损坏输入、
