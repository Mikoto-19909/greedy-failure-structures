...
原始图、每预算结果与完整诊断位于 [graphs](../results/r2_grid_v1/graphs/)，
逐预算数据见 [budget_results.csv](../results/r2_grid_v1/budget_results.csv)，
65 个参数单元的估计和区间见 [cell_summary.csv](../results/r2_grid_v1/cell_summary.csv)。
...
![R2 预算曲线](../results/r2_grid_v1/budget_curves.svg)
...
[机制汇总](../results/r2_grid_v1/mechanism_summary.csv)覆盖每单元预定前 32 张原图，
...
用于提出 R3 问题的探索选例是 [n12/d3/r0001](../results/r2_grid_v1/graphs/r2-exploration-n12-d3-r0001.json)。
...
[原图验证](../results/r2_grid_v1/verification.json)重算所有预定 O、规范见证、Greedy、
结构和完整诊断；[汇总验证](../results/r2_grid_v1/summary_verification.json)复算
...
R2/R3 探测的必需验证均实际执行，没有跳过。日志见[完整检查](../results/r2_r3_full_check.log)。
...
见[后续检查日志](../results/r3_confirmation_full_check.log)。修复验证仅使用临时样例，原始 R2 数据保留。
