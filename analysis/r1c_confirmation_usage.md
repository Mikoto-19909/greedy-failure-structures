...
正式 3,000 对样本已完成并通过独立验证，见[结果报告](r1c_confirmation_report.md)
与[归档数据](../experiments/r1c_confirmation_v1/)。
...
以下命令可复现正式批次。先记录实际源码提交、Python/SciPy 版本和完整命令，
按设计保留在运行日志或研究报告中。已有正式输出时使用新的复现目录；
也可直接使用归档中的 `config.json` 和原始 CSV 重建分析，具体命令见结果报告。
