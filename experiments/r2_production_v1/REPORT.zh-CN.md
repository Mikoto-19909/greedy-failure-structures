...
正式源码固定为 `0eda797c661317a11bac54eb52e7b0c722a54b04`。九图先导三组通过后，全量三种方式各执行三次新输出；全量总耗时中位数最小的是 `numba`。

## 实测结果

全部方式使用同一 CUDA 依赖环境，验证后端统一 NumBa 四进程；Python/NumBa 生产四进程，CUDA 使用单一隔离拥有者并串行执行 CPU 诊断。各阶段分别取中位数，阶段中位数之和不一定等于总中位数。

- python：生产 65.890 s；验证与分析 74.716 s；汇总校验 3.087 s；总中位数 **156.491 s**（133.958–156.872 s）。
- numba：生产 17.638 s；验证与分析 65.688 s；汇总校验 3.150 s；总中位数 **86.547 s**（85.318–99.167 s）。
- cuda：生产 51.222 s；验证与分析 64.830 s；汇总校验 3.115 s；总中位数 **119.175 s**（118.614–132.634 s）。
- numba 相对本轮 Python 基线：1.808 倍，总耗时减少 44.70%。
- cuda 相对本轮 Python 基线：1.313 倍，总耗时减少 23.85%。

九图先导是单次正确性/启动成本检查，不能作为重复性能统计：

- python：总 6.590 s，生产 1.812 s。
- numba：总 7.325 s，生产 2.465 s。
- cuda：总 7.256 s，生产 2.441 s。
...
- 累计测量墙钟 18.50 分钟。边界是 run → analyze --no-plot → summaries-only，包含每阶段进程/依赖启动、JIT 缓存载入及文件 I/O；先导预热了磁盘缓存。跨运行额外比对不计入。
