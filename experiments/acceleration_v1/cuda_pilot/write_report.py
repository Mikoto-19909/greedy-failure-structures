"""Render the local prototype findings from recorded measurements."""
import json
...
for row in data['summaries']:
    descriptions = []
    for key, label in [('python', '原始 Python'), ('cpu_compiled', '编译 CPU'), ('cuda', 'CUDA')]:
        values = row[key]
        descriptions.append(f"{label} {values['median']*1000:.3f} ms（范围 {values['min']*1000:.3f}–{values['max']*1000:.3f} ms）")
    lines.append(f"- m={row['m']}，batch={row['batch']}：" + '；'.join(descriptions) + f"。CUDA 相对原始 Python 为 {row['cuda_speedup_vs_python']:.2f} 倍，相对编译 CPU 为 {row['cuda_speedup_vs_compiled_cpu']:.2f} 倍（小于 1 表示 CUDA 更慢）。")
lines += ['', '## 正确性与实际限制', '',
    f"- 独立校验调用 `validate_r2_budget_grid.reference`，使用 Python set 与 itertools.combinations：{len(validation['fixtures_and_graphs'])} 个实例/边界样例，每个后端共 {sum(r['budgets_checked'] for r in validation['fixtures_and_graphs'])} 个预算结果，覆盖量和完整最优索引元组全部一致。",
    ...
    f"- 实际记录的首次调用：共享依赖导入 {data['cold_start']['imports_seconds']:.3f} s，编译 CPU 首次调用 {data['cold_start']['cpu_compiled_first_call_seconds']:.3f} s，CUDA 首次调用 {data['cold_start']['cuda_first_call_seconds']:.3f} s。它们按顺序测量，不能视为三个独立冷启动程序的完整耗时；后续重跑可能命中缓存。", '',
    '## 复现', '',
